...
Translator.prototype.aliasList = function(listId, alias) {
    var cachePath = this.config.get("configPath") + this.config.get("translationCache");
    this.cache.translations.lists[listId].alias = alias.toLowerCase();
    fs.writeFileSync(cachePath, JSON.stringify(this.cache));
};

Translator.prototype.getListIdByAlias = function(alias) {
    var lists = this.cache.translations.lists;
    for (var i in lists) {
        if (lists[i]["alias"] === alias.toLowerCase())
            return i;
    }
